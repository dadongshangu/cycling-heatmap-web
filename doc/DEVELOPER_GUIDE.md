...
**版本号格式：**
- 格式：`主版本号.次版本号`（如 `1.01`、`1.02`）
- 初始版本：`1.00`
- 递增规则：每次成功push后自动递增 `0.01`

**自动递增流程：**
1. 开发者执行 `git push`
2. pre-push hook运行所有测试
3. 测试通过后，检查版本号是否需要更新
4. 如果需要更新，自动调用 `bump-version.js` 更新版本号
5. 自动创建commit包含版本号更新
6. push包含版本号更新的commit
...
**智能检查：**
- 如果未推送的commit中已包含VERSION更新，则跳过以避免重复
- 如果工作区有未提交的VERSION更改，则跳过以避免覆盖用户更改

**手动更新：**
如果需要手动更新版本号，可以编辑 `VERSION` 文件或运行 `node scripts/bump-version.js`，但建议让系统自动管理。
